...
class SimpleHeaterAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        this.currentTemperatureOffset = parseInt(this.device.context.currentTemperatureOffset) || 0;
        ...
        const characteristicCurrentTemperature = service.getCharacteristic(Characteristic.CurrentTemperature)
            .updateValue(this._getOffsetDividedState(dps[this.dpCurrentTemperature], this.temperatureDivisor))
            .on('get', this.getOffsetDividedState.bind(this, this.dpCurrentTemperature, this.temperatureDivisor));
        ...
        this.device.on('change', (changes, state) => {
            ...
            if (changes.hasOwnProperty(this.dpCurrentTemperature) && characteristicCurrentTemperature.value !== changes[this.dpCurrentTemperature] + this.currentTemperatureOffset) characteristicCurrentTemperature.updateValue(changes[this.dpCurrentTemperature] + this.currentTemperatureOffset);

            // console.log('[TuyaAccessory] SimpleHeater changed: ' + JSON.stringify(state));
        });
    }

    getOffsetDividedState(dp, divisor, callback) {
        this.getState(dp, (err, data) => {
            if (err) return callback(err);
            if (!isFinite(data)) return callback(true);

            callback(null, this._getOffsetDividedState(data, divisor));
        });
    }


    _getOffsetDividedState(value, divisor) {
        return ((parseFloat(value) / divisor) || 0) + this.currentTemperatureOffset;
    }
