...
class TuyaAccessory extends EventEmitter {
    constructor(props) {
        ...
        if (this.context.version >= 3.2) {
            ...
            console.log(`[TuyaAccessory] Changing ping gap for ${this.context.name} to ${this.context.pingGap}s`);
        }
    ...
    _connect() {
        ...
        this._socket.on('error', err => {
            ...
            if (err) {
                if (err.code === 'ENOBUFS') {
                    console.warn('[TuyaAccessory] Operating system complained of resource exhaustion; did I open too many sockets?');
                    console.log('[TuyaAccessory] Slowing down retry attempts; if you see this happening often, it could mean some sort of incompatibility.');
                    delay = 60000;
    ...
    _msgHandler_3_1(task, callback) {
        ...
        if (this.context.intro === false && cmd !== 9)
            console.log('[TuyaAccessory] Message from', this.context.name + ':', data);
        ...
        switch (cmd) {
            ...
            case 8:
                ...
                try {
                    ...
                    data = decryptedMsg;
                    console.log(`[TuyaAccessory] Odd message from ${this.context.name} with command ${cmd}:`, data);
                    console.log(`[TuyaAccessory] Raw message from ${this.context.name} (${this.context.version}) with command ${cmd}:`, task.msg.toString('hex'));
                    break;
                }

                if (data && data.dps) {
                    //console.log('[TuyaAccessory] Update from', this.context.name, 'with command', cmd + ':', data.dps);
            ...
            case 10:
                if (data) {
                    if (data === 'json obj data unvalid' || data === 'data format error') {
                        console.log(`[TuyaAccessory] ${this.context.name} (${this.context.version}) didn't respond with its current state.`);
                        this.emit('change', {}, this.state);
                        break;
                    }
                    ...
                    try {
                        ...
                        console.log(`[TuyaAccessory] Malformed update from ${this.context.name} with command ${cmd}:`, data);
                        console.log(`[TuyaAccessory] Raw update from ${this.context.name} (${this.context.version}) with command ${cmd}:`, task.msg.toString('hex'));
                        break;
                    }
    ...
    _msgHandler_3_3(task, callback) {
        ...
        if (cmd === 10 && (decryptedMsg === 'json obj data unvalid' || decryptedMsg === 'data format error')) {
            console.log(`[TuyaAccessory] ${this.context.name} (${this.context.version}) didn't respond with its current state.`);
            this.emit('change', {}, this.state);
            return callback();
        }
        ...
        try {
            ...
            console.log(`[TuyaAccessory] Odd message from ${this.context.name} with command ${cmd}:`, decryptedMsg);
            console.log(`[TuyaAccessory] Raw message from ${this.context.name} (${this.context.version}) with command ${cmd}:`, task.msg.toString('hex'));
            return callback();
        }
    ...
    update(o) {
        ...
        if (hasDataPoint) {
            ...
            if (this.context.sendEmptyUpdate) {
                console.log("[TuyaAccessory] Sending", this.context.name, 'empty signature');
                this._send({cmd: 7});
            }
        } else {
            console.log(`[TuyaAccessory] Sending first query to ${this.context.name} (${this.context.version})`);
            result = this._send({
                data: {
